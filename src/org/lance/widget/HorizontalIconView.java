...
/**
 * 这是一个完全自定义的视图,只是一个处女作,不过它完成了几乎大部分自定义滚动视图所需要的操作
 * 这可以增强Android视图的理解,关于容器会更简单一点
 * @author ganchengkai
 *
 */
...
public class HorizontalIconView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		for(int i=0;i<iconCount;i++){
			...
			//记录绘制图标的位置---以此来跟踪点击事件
			final int drawnPosition = i -mSkippedIconCount;//首次绘制的位置
		...
		if(mEdgeEffectLeft!=null){
			if(mEdgeEffectLeft.isFinished()==false){
				//绘制光晕之后要恢复到上一状态
	...
	public void computeScroll() {
		if(mScroller.computeScrollOffset()){//判断是否结束滚动--true-动画没有结束
			...
			if(oldX!=x){
				...
				onScrollChanged(x,0,oldX,0);//通知视图执行了滚动---应用滚动
	...
	@Override //这是滚动处理最难的部分---需要跟踪所有变量所处的状态
			...
			if(mIsBeginDragged){//手势事件--计算速率
				...
				for(int i=0;i<mIconPositions.size();i++){
					...
					if(rect.contains(x,y)){
						...
						//得到点击的位置,回调到主界面
						Log.i(TAG, "Clicked position "+position+"; rect count:"+mIconPositions.size());
						if(mListener!=null){
